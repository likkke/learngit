...
class WebOfScienceItem_author_Pipeline(object):
    def __init__(self):
    ...
    def process_item(self, item, spider):
        '''先判断itme类型，在放入相应数据库'''
        if isinstance(item, Basic_information_of_the_paper_author_Item):
            try:
        ...
        if isinstance(item, Basic_information_of_the_quoted_paper_author_Item):
            try:
